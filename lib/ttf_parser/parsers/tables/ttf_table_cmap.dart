...
class TtfTableCmap implements TtfTable {
  ...
  var charToGlyphIndexMap = <int, int>{};
  var glyphToCharIndexMap = <int, int>{};
  int? languageID;
...
//      print ("Processing sub table with format: $format");
  ...
  void parseSubtableFormat4(StreamReader reader) { 
    ...
    if (segCountX2 % 2 != 0) {
      throw ParseException("Invalid cmap format 4: segCountX2 must be even, got $segCountX2");
    }

    int segCount = segCountX2 ~/ 2;
    
    if (segCount == 0 || segCount > 10000) {
      throw ParseException("Invalid cmap format 4: segCount out of range (0-10000): $segCount");
    }
    ...
    if (reservedPad != 0) {
      throw ParseException("Failed to parse cmap subtable format 4: reserved padding must be 0, got $reservedPad");
    }
...
//        print ("Char to Glyph mapping: $c <=> $glyphIndex");
  ...
  void parseSubtableFormat6(StreamReader reader) {
    ...
    if (entryCount > 65536) {
      throw ParseException("Invalid cmap format 6: entryCount too large: $entryCount");
    }
    
    for (var i = 0; i < entryCount; i++) {
      int charCode = firstCode + i;
      if (charCode > 65535) {
        throw ParseException("Invalid cmap format 6: character code out of range: $charCode");
      }
...
//      print ("Char to Glyph mapping: $charCode <=> $glyphIndex");
  ...
  void parseSubtableFormat0(StreamReader reader) {
    ...
    int length = reader.readUnsignedShort();
    if (length != 262) {
      throw ParseException("Invalid cmap subtable format 0: expected length 262, got $length");
    }
